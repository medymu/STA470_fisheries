...
Next, to understand the effect of time before adding other Level One covariates, the model that only includes year and month as Level One predictors was fitted (Table 3). The Pseudo $R^2$ estimating the change in within-orgnization variance between the unconditional means and the Model with Time is about 0.12. Therefore, understanding changes of time (i.e. years) accounts for 12% variability in logcpue.

```{r, cache=TRUE}
model_time <- lmer(logcpue ~ yearMonth +  (yearMonth|uniqueid) + (yearMonth|fed),
                  data = x, na.action = na.omit)
tidy(model_time) %>%
...
The final model we come up with removes variance terms associated with level Three slopes since adding them makes the model too complex. Age was only included in level Three intercept since we hypothesized that the interaction between the age of federation with our level One predictors of interests are not strong.

```{r, cache=TRUE}
final_model <- lmer(logcpue ~ yearMonth + region + taxa + age 
                    + (yearMonth + region + taxa|uniqueid) + (1|fed), 
                            data = x, na.action = na.omit)

tidy(final_model) %>%
  kable(digits = 3, caption = "Final Model")
```

```{r, cache=TRUE}
glance(final_model) %>% 
  select(AIC, BIC) %>%
  kable(digits = 0, caption = "AIC and BIC for Final Model")
```
...
$$ Logcpue_{ijk} = a_i + u_{ij} + b_i yearmonth_{ijk} + v_{ij} yearmonth_{ijk} + c_i region_{ijk} + w_{ij} region_{ijk} $$
$$+ d_i taxa_{ijk} + x_{ij} taxa_{ijk} + \epsilon_{ijk}, \space \epsilon_{ijk} \sim N(0, \sigma^2)$$


$$ = \alpha_0 + \alpha_1 age_{i} + \tilde u_{i} + u_{ij} + \beta_0 yearmonth_{ijk} + v_{ij} yearmonth_{ijk} $$
$$+ \gamma_0 region_{ijk} + w_{ij} region_{ijk} + \delta_0 taxa_{ijk}+ x_{ij} taxa_{ijk} + \epsilon_{ijk}, \space \epsilon_{ijk} \sim N(0, \sigma^2)$$
$$ = \alpha_0 + \alpha_1 age_{i} + \beta_0 yearmonth_{ijk} + \gamma_0 region_{ijk} + \delta taxa_{ijk} $$
$$+ [u_{ij} + v_{ij} yearmonth_{ijk} + w_{ij} region_{ijk} +  x_{ij} taxa_{ijk} + \tilde u_{i} + \epsilon_{ijk}] $$
$$\epsilon_{ijk} \sim N(0, \sigma^2)$$

```{r, cache=TRUE}
anova(final_model)
par(mfrow=c(2,2),las=1)
plot(final_model,scale=0)
```

```{r, cache=TRUE}
x <- x %>% 
   mutate(pred_logcpue = predict(final_model, x, re.form = NULL))
```
